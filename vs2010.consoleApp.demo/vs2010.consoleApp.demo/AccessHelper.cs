...
namespace vs2010.consoleApp.demo
{
        ...
        public class AccessHelper
        {
            ...
            public static OleDbConnection OpenConnection(String connections)
            {
            ...
            private static void PrepareCommand(OleDbCommand command, OleDbConnection connection,
                                               OleDbTransaction transaction, string commandText,
                                               OleDbParameter[] commandParameters, out bool mustCloseConnection)
            {
                ...
                if (null != transaction)
                {
                    if (null == transaction.Connection)
                        throw new ArgumentException("transaction对象commited和rollbacked需要一个已打开的事务", "transaction");
                    ...
                    foreach (OleDbParameter p in commandParameters)
                    {
                        if (null != p)
                        {
                            if ((ParameterDirection.Input == p.Direction ||
                                 ParameterDirection.InputOutput == p.Direction) && (null == p.Value))
                            {
            ...
            {
                if (null == connectionString || string.Empty == connectionString)
                                              ...
                                              params OleDbParameter[] commandParameters)
            {
                ...
                PrepareCommand(cmd, connection, (OleDbTransaction) null, commandText, commandParameters,
            ...
            public static OleDbDataReader ExecuteReader(OleDbConnection connection, string commandText,
                                                        params OleDbParameter[] commandParameters)
            {
                return ExecuteReader(connection, (OleDbTransaction) null, commandText, commandParameters,
