...
namespace URLRewriter
{
	...
	internal class RewriterUtils
	{
		...
		internal static void RewriteUrl(HttpContext context, string sendToUrl, out string sendToUrlLessQString, out string filePath)
		{
            ...
            //filePath = sendToUrlLessQString;
            filePath = context.Server.MapPath(sendToUrlLessQString);//不必映射到物理路径，可以用虚拟路径！这是为HttpFactory使用的
			...
			else
			{
				...
				if (url[1] == '/' || url[1] == '\\')
				{
					...
					if (appPath.Length > 1)
						...
						return "/" + url.Substring(2);//一般返回的是这个路径，请求路径格式为/test.aspx这种虚拟路径形式
				}
				else
				{
