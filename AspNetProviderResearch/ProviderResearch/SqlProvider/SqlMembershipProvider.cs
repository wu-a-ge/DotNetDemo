    ...
    public class SqlMembershipProvider : MembershipProvider
    {
        ...
        public override MembershipUser CreateUser(string username, string password, string email, string passwordQuestion, string passwordAnswer, bool isApproved, object providerUserKey, out MembershipCreateStatus status)
        {
            ...
            if (!string.IsNullOrEmpty(passwordAnswer))
            {
                ...
                //加密密码提示问题答案
                pass = base.EncodePassword(passwordAnswer.ToLower(CultureInfo.InvariantCulture), (int) this._PasswordFormat, salt);
            }
            //为空
            else
            {
            ...
            //为什么已经加密才去验证参数？应该先验证了再加密！像用户密码一样
            if (!SecUtility.ValidateParameter(ref pass, this.RequiresQuestionAndAnswer, true, false, 0x80))
            {
                status = MembershipCreateStatus.InvalidAnswer;
                return null;
            }
            ...
            try
            {
                ...
                try
                {
                    ...
                    command.Parameters.Add(this.CreateInputParam("@PasswordAnswer", SqlDbType.NVarChar, pass));
                    ...
                    command.ExecuteNonQuery();
                    //用返回值来判断状态
                    int statusValue = (parameter.Value != null) ? ((int) parameter.Value) : -1;
                    if ((statusValue < 0) || (statusValue > 11))
                    {
                        statusValue = 11;
                    }
                    status = (MembershipCreateStatus)statusValue;
                    if (statusValue != 0)
                    {
                        return null;
                    }
                    ...
                    //构建用户对象 
                    user = new MembershipUser(this.Name, username, providerUserKey, email, passwordQuestion, null, isApproved, false, time, time, time, time, new DateTime(1754, 1, 1));
                }
                finally
                {
                    if (connection != null)
